...
od_user_list.each { |line|
  ...
  num_errors += $?.exitstatus
  system "#{dscl_command} create Users/#{user_name} RealName '#{name}'"
  num_errors += $?.exitstatus
  ...
  if num_errors == 0
    ...
    output_users.puts "#{user_name}\t#{pass}\t#{name}\t#{whatever_else}"
